...
export default function Hero({ styles, currentYear }: HeroProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <main className="relative md:absolute md:inset-0 flex flex-col justify-center px-6 md:px-12 z-10 py-6 md:py-0">
        <div className="max-w-7xl mx-auto w-full">
          ...
          >
            <span className="text-sm md:text-2xl font-medium drop-shadow-md leading-snug">Hey👋, I build high-performance web apps that scale, convert, and make businesses money.</span>
          </motion.div>
          ...
          <div className="relative">
            <motion.h1
              ...
              className="text-[14vw] md:text-[10vw] leading-[0.9] font-bold tracking-tighter text-[#a3ff33] uppercase drop-shadow-[0_10px_30px_rgba(163,255,51,0.2)]"
            >
              SAIDUL ALOM
            </motion.h1>
            ...
            <motion.div 
              ...
              className="flex flex-row flex-wrap gap-2 md:gap-4 mt-5 md:mt-8"
            >
              <button 
                onClick={() => {
                  const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
                  window.scrollTo({ top: (3 / 7) * maxScroll, behavior: "smooth" });
                }}
                className="btn-glow px-5 py-2.5 md:px-8 md:py-3 bg-[#a3ff33] text-black font-bold uppercase tracking-widest text-[10px] md:text-xs rounded-full hover:bg-white transition-all shadow-[0_0_20px_rgba(163,255,51,0.3)]"
              >
                View Projects
              </button>
              <button 
              ...
              <button
                ...
                className="btn-glow px-5 py-2.5 md:px-8 md:py-3 bg-transparent border border-white/30 text-white font-bold uppercase tracking-widest text-[10px] md:text-xs rounded-full hover:border-white transition-all"
              >
                Resume
              </button>
            </motion.div>
            ...
            <motion.div 
              ...
              className="h-[1px] bg-[#a3ff33]/30 mt-5 md:mt-8"
            />
          </div>
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 1, delay: 0.8 }}
            className="mt-4 md:mt-6 text-xs md:text-sm text-gray-400 font-medium flex items-center gap-4"
          >
      ...
      <motion.div 
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 2 }}
        className="hidden md:flex absolute bottom-12 left-1/2 -translate-x-1/2 z-20 flex-col items-center gap-4"
      >
      ...
      <footer className="hidden md:flex md:absolute md:bottom-0 left-0 right-0 p-8 md:p-12 flex-col md:flex-row justify-between items-end md:items-center gap-8 z-20">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
